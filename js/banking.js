...
function updateValue(id,inputAmount){
    ...
    const previousAmount = parseInt(previousText);
...
function updateBalance(inputAmount,isAdd){
    ...
    if(isAdd){
        previousBalanceField.innerText = inputAmount + previousBalance;
    }else{
        previousBalanceField.innerText = previousBalance-inputAmount;
    }
}
